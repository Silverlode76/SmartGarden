...
LoRa_Device newLoRaDevice(int frequency, uint8_t spreadingFactor, uint8_t bandWidth, uint8_t crcRate, uint16_t preamble, uint8_t power, uint8_t	overCurrentProtection)
{
	LoRa_Device new_LoRa_Device;
	new_LoRa_Device.myLoRa.frequency = frequency;
	new_LoRa_Device.myLoRa.spreadingFactor = spreadingFactor;
	new_LoRa_Device.myLoRa.bandWidth = bandWidth;
	new_LoRa_Device.myLoRa.crcRate = crcRate;
	new_LoRa_Device.myLoRa.preamble = preamble;
	new_LoRa_Device.myLoRa.power = power;
	new_LoRa_Device.myLoRa.overCurrentProtection = overCurrentProtection;

	return new_LoRa_Device;
...
void LoRa_setAutoLDO(LoRa* _LoRa){
	...
	LoRa_setLowDaraRateOptimization(_LoRa, (long)((1 << _LoRa->spreadingFactor) / ((double)BW[_LoRa->bandWidth])) > 16.0);
}
...
uint8_t LoRa_getOpModeStatus(LoRa *myLoRa) {
    uint8_t regValue = LoRa_read(myLoRa, RegOpMode);
    return regValue & 0x07;  // Die unteren 3 Bits geben den Modus an
}

uint8_t LoRa_isBusy(LoRa *myLoRa){

	if (LoRa_getOpModeStatus(myLoRa) == 0x00) {  // Prüfe, ob das Modul im Empfangsmodus ist
...
void LoRa_send(LoRa *myLoRa, uint8_t *data, uint8_t length) {
    // 1. Sicherstellen, dass der SX1278 bereit ist
    while (LoRa_isBusy(myLoRa)) {
        HAL_Delay(1);
    }

    // 2. Empfangs-Interrupt deaktivieren (optional, falls nötig)
    HAL_NVIC_DisableIRQ(EXTI0_IRQn); // Beispiel für EXTI0_IRQn

    // 3. Modul in den Sende-Modus versetzen
    LoRa_gotoMode(myLoRa, TRANSMIT_MODE);

    // 4. Daten senden
    LoRa_BurstWrite(myLoRa,RegFiFo, data, length);

    HAL_Delay(1);

    // 7. Modul wieder in den Empfangsmodus versetzen
	LoRa_gotoMode(myLoRa, RXCONTIN_MODE );

    // 8. Empfangs-Interrupt wieder aktivieren (falls deaktiviert)
    HAL_NVIC_EnableIRQ(EXTI0_IRQn);
}

void configureDIO0ForTxDone(LoRa *myLoRa) {
    uint8_t regValue = LoRa_read(myLoRa, RegDioMapping1);
    regValue &= 0x3F;    // Reset Mapping (Bits 7-6 for DIO0)
    regValue |= 0x40;    // Set to '01' for TxDone
    LoRa_write(myLoRa, RegDioMapping1, regValue);
}

void configureDIO0ForRxDone(LoRa *myLoRa) {
    uint8_t regValue = LoRa_read(myLoRa, RegDioMapping1);
    regValue &= 0x3F;    // Reset Mapping (Bits 7-6 for DIO0)
    regValue |= 0x00;    // Set to '00' for RxDone
    LoRa_write(myLoRa, RegDioMapping1, regValue);
}
